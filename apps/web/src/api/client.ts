...
export async function getHealth(): Promise<HealthResponse> {
  const response = await fetch(`${API_BASE}/ops/health`);
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    const detail = errorData.detail || response.statusText;
    throw new ApiError(`Health check failed: ${detail}`, response.status, detail);
  }
  return response.json();
}
...
export async function getOpsStatus(): Promise<OpsStatusResponse> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    const detail = errorData.detail || response.statusText;
    throw new ApiError(`Failed to get ops status: ${detail}`, response.status, detail);
  }
  return response.json();
}
