...
between runs makes the Insights page feel unreliable — the same weakness would
be called something different each week. The AI stage may personalise the
*explanation* on an individual diagnosis; the pattern's identity stays fixed.
...
is. Persisting it would buy a job, two tables, a migration and a staleness
problem for a computation that takes milliseconds — see the module note at the
bottom for what would justify revisiting that.
"""
...
#
# Descriptions are written in second person and describe the *habit*, not the
# position — the user already knows what happened on the board; what they need
...
_PATTERNS: dict[tuple[str, str | None], tuple[str, str]] = {
    ...
    ("recapture_assumption", None): (
        "Automatic Recapture",
        "When something gets taken, you take back without stopping to look. "
        "A recapture is a move like any other — check the in-between options "
        "before playing it.",
    ),
    ...
    ("king_safety_blindness", None): (
        ...
        "enemy pieces point at it, count the attackers before continuing "
        "your own plan.",
    ),
    ...
    ("opening_pattern_gap", None): (
        "Opening Pattern Gap",
        "You go wrong early in positions that have a known handling. These "
        "are worth learning rather than calculating.",
    ),
...
def identify(cause: str, phase: str | None) -> PatternIdentity | None:
    ...
    dressing it up as one would be the exact overreach this feature avoids.
    """
...
def priority_score(mistakes: int, accuracy: float | None, recent: int) -> float:
    """How much attention a pattern deserves, relative to the others.
    ...
    * **difficulty** — a pattern you already solve when retried needs less work
      than one you keep failing; unknown accuracy is treated as neutral rather
      than as either extreme, so an untested pattern neither jumps the queue
      nor gets buried
    * **recency** — a habit still showing up in recent games matters more than
      one you may already have grown out of

    Not a probability and not comparable across users — it exists to order one
    person's patterns against each other, nothing more.
    """
    difficulty = 1.0 if accuracy is None else 1.0 + (1.0 - accuracy)
    recency = 1.0 + (recent / mistakes if mistakes else 0.0)
    return round(mistakes * difficulty * recency, 3)


# Why no cluster tables
# --------------------
...
# millisecond query, so the tables would add a migration, a job type, and a
# staleness window in exchange for nothing measurable.
#
# What would justify revisiting: corpora large enough that the grouping query
# shows up in request timings, or clustering that stops being derivable from
