...
def main():
    ...
    print(
        "Expected: diagnosis card does NOT appear (that is the bug being fixed)",
        flush=True,
    )
    ...
    with sync_playwright() as pw:
        ...
        page.goto(origin)
        page.evaluate(
            "localStorage.setItem('knightmind:chesscom_username', 'testplayer')"
        )
        ...
        diagnosis_present = (
            page.locator('[data-testid="post-resolution-diagnosis"]').count() > 0
        )
        loose_present = page.locator("text=Loose piece awareness").count() > 0

        page.screenshot(
            path=str(ARTIFACTS / "prefixed_desktop_state.png"), full_page=True
        )
    ...
    if not diagnosis_present and not loose_present:
        ...
        print(
            "\nRED CONFIRMED: Diagnosis card absent on pre-fix build (expected).",
            flush=True,
        )
        ...
        print(
            f"\nUNEXPECTED: pre-fix bundle showed diagnosis. Investigate. {result_path}",
            flush=True,
        )
        sys.exit(1)
